...
[
	'title' => _i('Main Goals'),
	'subtitle' => _i('Main Goals'),
	'activePageName' => _i('Main Goals'),
	'activePageUrl' => route('main_goals.index'),
	'additionalPageName' =>  _i('Main Goals'),
	'additionalPageUrl' => route('main_goals.index')
])
@section('content')
<div class="box-body">
	<div class="row">
        @can('Create_main_goals')
		<div class="col-sm-12 mb-3">
			<span class="pull-left">
			<button class="btn btn-primary create add-permissiont" type="button" data-toggle="modal"
				data-target="#modal-default">
				<span><i class="ti-plus"></i> {{_i('Create new Main Goals')}} </span>
			</button>
			</span>
		</div>
        @endcan
...
@include('admin.main_goals.modal')
@endsection
@push('js')
<script>
	...
	{
		e.preventDefault();
		let url = "{{ route('main_goals.update') }}";
		$.ajax({
			url: url,
			method: "post",
			data: new FormData(this),
    ...
    //    alert(transRowId);
		$.ajax({
			url: '{{route('main_goals.get.translation')}}',
			method: "get",
			"_token": "{{ csrf_token() }}",
			data: {
				'lang_id': lang_id,
