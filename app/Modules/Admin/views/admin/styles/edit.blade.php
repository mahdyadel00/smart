				...
				<div class="col-sm-12 ">
					<div class="card">
						<div class="card-block">
							<div class="card-body">
								<form method="post" action="{{route('admin.styles.update')}}" id="form_idd" data-parsley-validate="">
									@csrf
									<div class="row  ">
										<input type="hidden" value="{{$setting_template->id}}" name="id" id="id">
										<div class="col-md-12 row">
											<div class="form-group col-sm-3   ">
												<label class="control-label">{{_i(' style name')}}</label>
												<input type="text" name="name" value="{{$style->name}}" class="form-control" required >

											</div>
											<div class="form-group col-sm-3   ">
												<label class="control-label">{{_i('Date From')}}</label>
												<input type="date" name="from" value="{{$style->start_date}}" class="form-control" required >

											</div>
											<div class="form-group col-sm-3 ">
												<label class="control-label">{{_i('Date To')}}</label>
												<input type="date" name="to" value="{{$style->end_date}}" class="form-control" required >

											</div>
											<div class="form-group mt-3 ">
												<label  >{{_i('Active')}}</label>
												<input type="checkbox" name="active" value="1"  @if($style->active == 1) checked @endif class="form-control">
									...
									<div class="box-footer">
										<button type="button" class="btn btn-primary Preview "   name="preview" value="preview" id="previeww" >{{_i('Preview')}}</button>

										<button type="submit" class="btn btn-primary save " form="form_idd"  name="save" id="save" value="save">{{_i('Save')}}</button>
									</div>
								</form>
							</div>
						</div>
...
@push('js')
	<script>
        ...
        $(document).on('click','#previeww',function (e) {
            e.preventDefault();

            var url = "{{route('admin.styles.update')}}";
